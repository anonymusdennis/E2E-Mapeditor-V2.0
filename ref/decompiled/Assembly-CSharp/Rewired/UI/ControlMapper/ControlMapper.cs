...
public class ControlMapper : MonoBehaviour
{
	...
	public class MappingSet
	{
		...
		[UnityEngine.Tooltip("The Map Category that will be displayed to the user for remapping.")]
		[SerializeField]
		private int _mapCategoryId;

		[SerializeField]
		[UnityEngine.Tooltip("Choose whether you want to list Actions to display for this Map Category by individual Action or by all the Actions in an Action Category.")]
	...
	{
		[SerializeField]
		[UnityEngine.Tooltip("The Input Behavior that will be displayed to the user for modification.")]
		private int _inputBehaviorId = -1;

		[SerializeField]
		[UnityEngine.Tooltip("If checked, a slider will be displayed so the user can change this value.")]
		...
		[UnityEngine.Tooltip("If set to a non-blank value, this key will be used to look up the name in Language to be displayed as the title for the Input Behavior control set. Otherwise, the name field of the InputBehavior will be used.")]
		[SerializeField]
		private string _labelLanguageKey = string.Empty;

		[UnityEngine.Tooltip("If set to a non-blank value, this name will be displayed above the individual slider control. Otherwise, no name will be displayed.")]
		[SerializeField]
		private string _joystickAxisSensitivityLabelLanguageKey = string.Empty;

		[UnityEngine.Tooltip("If set to a non-blank value, this key will be used to look up the name in Language to be displayed above the individual slider control. Otherwise, no name will be displayed.")]
		[SerializeField]
		private string _mouseXYAxisSensitivityLabelLanguageKey = string.Empty;

		[UnityEngine.Tooltip("The icon to display next to the slider. Set to none for no icon.")]
		[SerializeField]
		...
		[UnityEngine.Tooltip("Minimum value the user is allowed to set for this property.")]
		[SerializeField]
		private float _joystickAxisSensitivityMin;

		[UnityEngine.Tooltip("Maximum value the user is allowed to set for this property.")]
		[SerializeField]
	...
	[UnityEngine.Tooltip("Must be assigned a Rewired Input Manager scene object or prefab.")]
	[SerializeField]
	private InputManager _rewiredInputManager;

	[UnityEngine.Tooltip("Set to True to prevent the Game Object from being destroyed when a new scene is loaded.\n\nNOTE: Changing this value from True to False at runtime will have no effect because Object.DontDestroyOnLoad cannot be undone once set.")]
	[SerializeField]
	private bool _dontDestroyOnLoad;

	[SerializeField]
	[UnityEngine.Tooltip("Open the control mapping screen immediately on start. Mainly used for testing.")]
	private bool _openOnStart;

	[UnityEngine.Tooltip("The Layout of the Keyboard Maps to be displayed.")]
	[SerializeField]
	private int _keyboardMapDefaultLayout;

	[SerializeField]
	[UnityEngine.Tooltip("The Layout of the Mouse Maps to be displayed.")]
	...
	[SerializeField]
	[UnityEngine.Tooltip("Display a selectable list of Players. If your game only supports 1 player, you can disable this.")]
	private bool _showPlayers = true;

	[UnityEngine.Tooltip("Display the Controller column for input mapping.")]
	[SerializeField]
	private bool _showControllers = true;

	[UnityEngine.Tooltip("Display the Keyboard column for input mapping.")]
	[SerializeField]
	private bool _showKeyboard = true;

	[SerializeField]
	[UnityEngine.Tooltip("Display the Mouse column for input mapping.")]
	private bool _showMouse = true;

	[SerializeField]
	[UnityEngine.Tooltip("The maximum number of controllers allowed to be assigned to a Player. If set to any value other than 1, a selectable list of currently-assigned controller will be displayed to the user. [0 = infinite]")]
	private int _maxControllersPerPlayer = 1;

	[UnityEngine.Tooltip("Display section labels for each Action Category in the input field grid. Only applies if Action Categories are used to display the Action list.")]
	[SerializeField]
	private bool _showActionCategoryLabels;

	[SerializeField]
	[UnityEngine.Tooltip("The number of input fields to display for the keyboard. If you want to support alternate mappings on the same device, set this to 2 or more.")]
	private int _keyboardInputFieldCount = 2;

	[UnityEngine.Tooltip("The number of input fields to display for the mouse. If you want to support alternate mappings on the same device, set this to 2 or more.")]
	[SerializeField]
	private int _mouseInputFieldCount = 1;

	[SerializeField]
	[UnityEngine.Tooltip("The number of input fields to display for joysticks. If you want to support alternate mappings on the same device, set this to 2 or more.")]
	private int _controllerInputFieldCount = 1;

	[UnityEngine.Tooltip("Display a full-axis input assignment field for every axis-type Action in the input field grid. Also displays an invert toggle for the user  to invert the full-axis assignment direction.\n\n*IMPORTANT*: This field is required if you have made any full-axis assignments in the Rewired Input Manager or in saved XML user data. Disabling this field when you have full-axis assignments will result in the inability for the user to view, remove, or modify these full-axis assignments. In addition, these assignments may cause conflicts when trying to remap the same axes to Actions.")]
	[SerializeField]
	private bool _showFullAxisInputFields = true;

	[UnityEngine.Tooltip("Display a positive and negative input assignment field for every axis-type Action in the input field grid.\n\n*IMPORTANT*: These fields are required to assign buttons, keyboard keys, and hat or D-Pad directions to axis-type Actions. If you have made any split-axis assignments or button/key/D-pad assignments to axis-type Actions in the Rewired Input Manager or in saved XML user data, disabling these fields will result in the inability for the user to view, remove, or modify these assignments. In addition, these assignments may cause conflicts when trying to remap the same elements to Actions.")]
	[SerializeField]
	private bool _showSplitAxisInputFields = true;

	[UnityEngine.Tooltip("If enabled, when an element assignment conflict is found, an option will be displayed that allows the user to make the conflicting assignment anyway.")]
	[SerializeField]
	private bool _allowElementAssignmentConflicts;

	[SerializeField]
	[UnityEngine.Tooltip("The width in relative pixels of the Action label column.")]
	private int _actionLabelWidth = 200;

	[UnityEngine.Tooltip("The width in relative pixels of the Keyboard column.")]
	[SerializeField]
	private int _keyboardColMaxWidth = 360;

	[UnityEngine.Tooltip("The width in relative pixels of the Mouse column.")]
	[SerializeField]
	private int _mouseColMaxWidth = 200;

	[SerializeField]
	[UnityEngine.Tooltip("The width in relative pixels of the Controller column.")]
	private int _controllerColMaxWidth = 200;

	[UnityEngine.Tooltip("The height in relative pixels of the input grid button rows.")]
	[SerializeField]
	private int _inputRowHeight = 40;

	[UnityEngine.Tooltip("The width in relative pixels of spacing between columns.")]
	[SerializeField]
	private int _inputColumnSpacing = 40;

	[SerializeField]
	[UnityEngine.Tooltip("The height in relative pixels of the space between Action Category sections. Only applicable if Show Action Category Labels is checked.")]
	private int _inputRowCategorySpacing = 20;

	[SerializeField]
	[UnityEngine.Tooltip("The width in relative pixels of the invert toggle buttons.")]
	private int _invertToggleWidth = 40;

	[UnityEngine.Tooltip("The width in relative pixels of generated popup windows.")]
	[SerializeField]
	private int _defaultWindowWidth = 500;

	[SerializeField]
	[UnityEngine.Tooltip("The height in relative pixels of generated popup windows.")]
	private int _defaultWindowHeight = 400;

	[UnityEngine.Tooltip("The time in seconds the user has to press an element on a controller when assigning a controller to a Player. If this time elapses with no user input a controller, the assignment will be canceled.")]
	[SerializeField]
	private float _controllerAssignmentTimeout = 5f;

	[SerializeField]
	[UnityEngine.Tooltip("The time in seconds the user has to press an element on a controller while waiting for axes to be centered before assigning input.")]
	private float _preInputAssignmentTimeout = 5f;

	[SerializeField]
	[UnityEngine.Tooltip("The time in seconds the user has to press an element on a controller when assigning input. If this time elapses with no user input on the target controller, the assignment will be canceled.")]
	private float _inputAssignmentTimeout = 5f;

	[UnityEngine.Tooltip("The time in seconds the user has to press an element on a controller during calibration.")]
	[SerializeField]
	private float _axisCalibrationTimeout = 5f;

	[UnityEngine.Tooltip("If checked, mouse X-axis movement will always be ignored during input assignment. Check this if you don't want the horizontal mouse axis to be user-assignable to any Actions.")]
	[SerializeField]
	private bool _ignoreMouseXAxisAssignment = true;

	[SerializeField]
	[UnityEngine.Tooltip("If checked, mouse Y-axis movement will always be ignored during input assignment. Check this if you don't want the vertical mouse axis to be user-assignable to any Actions.")]
	private bool _ignoreMouseYAxisAssignment = true;

	[SerializeField]
	[UnityEngine.Tooltip("An Action that when activated will alternately close or open the main screen as long as no popup windows are open.")]
	private int _screenToggleAction = -1;

	[SerializeField]
	[UnityEngine.Tooltip("An Action that when activated will open the main screen if it is closed.")]
	private int _screenOpenAction = -1;

	[UnityEngine.Tooltip("An Action that when activated will close the main screen as long as no popup windows are open.")]
	[SerializeField]
	private int _screenCloseAction = -1;

	[SerializeField]
	[UnityEngine.Tooltip("An Action that when activated will cancel and close any open popup window. Use with care because the element assigned to this Action can never be mapped by the user (because it would just cancel his assignment).")]
	private int _universalCancelAction = -1;

	[UnityEngine.Tooltip("If enabled, Universal Cancel will also close the main screen if pressed when no windows are open.")]
	[SerializeField]
	private bool _universalCancelClosesScreen = true;

	[SerializeField]
	[UnityEngine.Tooltip("If checked, controls will be displayed which will allow the user to customize certain Input Behavior settings.")]
	private bool _showInputBehaviorSettings;

	[UnityEngine.Tooltip("Customizable settings for user-modifiable Input Behaviors. This can be used for settings like Mouse Look Sensitivity.")]
	[SerializeField]
	private InputBehaviorSettings[] _inputBehaviorSettings;

	[UnityEngine.Tooltip("If enabled, UI elements will be themed based on the settings in Theme Settings.")]
	[SerializeField]
	private bool _useThemeSettings = true;

	[UnityEngine.Tooltip("Must be assigned a ThemeSettings object. Used to theme UI elements.")]
	[SerializeField]
	private ThemeSettings _themeSettings;

	[UnityEngine.Tooltip("Must be assigned a LanguageData object. Used to retrieve language entries for UI elements.")]
	[SerializeField]
	private LanguageData _language;

	[UnityEngine.Tooltip("A list of prefabs. You should not have to modify this.")]
	[SerializeField]
	private Prefabs prefabs;

	[UnityEngine.Tooltip("A list of references to elements in the hierarchy. You should not have to modify this.")]
	[SerializeField]
	private References references;

	[SerializeField]
	[UnityEngine.Tooltip("Show the label for the Players button group?")]
	private bool _showPlayersGroupLabel = true;

	[UnityEngine.Tooltip("Show the label for the Controller button group?")]
	[SerializeField]
	private bool _showControllerGroupLabel = true;

	[SerializeField]
	[UnityEngine.Tooltip("Show the label for the Assigned Controllers button group?")]
	private bool _showAssignedControllersGroupLabel = true;

	[UnityEngine.Tooltip("Show the label for the Settings button group?")]
	[SerializeField]
	private bool _showSettingsGroupLabel = true;

	[SerializeField]
	[UnityEngine.Tooltip("Show the label for the Map Categories button group?")]
	private bool _showMapCategoriesGroupLabel = true;

	[UnityEngine.Tooltip("Show the label for the current controller name?")]
	[SerializeField]
	private bool _showControllerNameLabel = true;

	[UnityEngine.Tooltip("Show the Assigned Controllers group? If joystick auto-assignment is enabled in the Rewired Input Manager and the max joysticks per player is set to any value other than 1, the Assigned Controllers group will always be displayed.")]
	[SerializeField]
	...
	private IEnumerable<ElementAssignmentConflictInfo> ElementAssignmentConflicts(Player player, InputMapping mapping, ElementAssignment assignment, bool skipOtherPlayers)
	{
		...
		if (skipOtherPlayers)
		{
			foreach (ElementAssignmentConflictInfo conflict2 in ReInput.players.SystemPlayer.controllers.conflictChecking.ElementAssignmentConflicts(conflictCheck))
			{

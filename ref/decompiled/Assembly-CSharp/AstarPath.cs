...
public class AstarPath : MonoBehaviour
{
	...
	public static NNConstraint m_NNnone = NNConstraint.None;

	public static Version Version => new Version(3, 8, 7);
	...
	public void SetTagNames(string[] replacementNames)
	{
		string[] array = GetTagNames();
		for (int i = 0; i < replacementNames.Length; i++)
		{
			array[i] = replacementNames[i];
		}
		tagNames = array;
	}
	...
	private void OnDrawGizmos()
	{
		...
		if (debugMode == GraphDebugMode.Tags && showNavGraphs)
		{
			GraphNodeDelegateCancelable del = DrawNodeTags;
		...
		if (debugMode == GraphDebugMode.DmgTiles && showNavGraphs)
		{
			GraphNodeDelegateCancelable del2 = DrawNodeDmgTiles;
			for (int l = 0; l < graphs.Length; l++)
			{
				if (graphs[l] != null && graphs[l].drawGizmos)
				{
					graphs[l].GetNodes(del2);
				}
			}
		}
		if (showUnwalkableNodes && showNavGraphs)
		{
			...
			GraphNodeDelegateCancelable del3 = DrawUnwalkableNode;
			for (int m = 0; m < graphs.Length; m++)
			{
				if (graphs[m] != null && graphs[m].drawGizmos)
				{
					graphs[m].GetNodes(del3);
				}
			}
		}
	...
	private bool DrawNodeTags(GraphNode node)
	{
		uint num = node.Tag;
		if (num != 0)
		{
			float h = (float)num / 10f;
	...
	private bool DrawNodeDmgTiles(GraphNode node)
	{
		bool flag = node.m_DamagableGroundTile != null;
		bool flag2 = node.m_DamagableWallTile != null;
		if (flag || flag2)
		{
	...
	public void Awake()
	{
		...
		int num = CalculateThreadCount(ThreadCount.AutomaticLowLoad);
	...
	public void Scan(bool bUpdateNetworkService = false)
	{
		ScanLoop(null, bUpdateNetworkService);
	}

	public void ScanLoop(OnScanStatus statusCallback, bool bUpdateNetworkService = false)
	{
		if (graphs == null)
		{
			return;
		...
		for (int i = 0; i < graphs.Length; i++)
		{
			...
			navGraph.ScanInternal(statusCallback2, bUpdateNetworkService);
			if (bUpdateNetworkService)
			{
				GlobalStart.TimedNetworkService();
			}
	...
	private static void CalculatePathsThreaded(object _threadInfo)
	{
		...
		{
			if (ex3 is ThreadAbortException || ex3 is ThreadControlQueue.QueueTerminationException)
			{
	...
	{
		return GetNearest(position, m_NNnone);
	}

...
public class PhotonView : Photon.MonoBehaviour
{
	...
	public bool RegisteredByTeam17 { get; set; }

	protected internal void Awake()
	{
		if (viewID != 0)
		{
			if (!RegisteredByTeam17)
			{
				PhotonNetwork.networkingPeer.RegisterPhotonView(this);
			}
			instantiationDataField = PhotonNetwork.networkingPeer.FetchInstantiationData(instantiationId);
	...
	protected internal void OnDestroy()
	{
		...
		ClearRpcMonoBehaviourCache();
		lastOnSerializeDataReceived = null;
	...
	{
		if (NetworkingPeer.m_bDiscardSerializeOnViews)
		{
			return;
		}
	...
	public void RefreshRpcMonoBehaviourCache()
	{
		...
		{
			UnityEngine.MonoBehaviour monoBehaviour = componentsInChildren[i];
			if (monoBehaviour != null)
			{
				T17NetView component = monoBehaviour.GetComponent<T17NetView>();
				if (component != null && component.viewID != num)
				{
		...
		RpcMonoBehaviours = new UnityEngine.MonoBehaviour[num3];
		int num4 = 0;
		for (int j = 0; j < num2; j++)
		{
			if (componentsInChildren[j] != null)
			{
				RpcMonoBehaviours[num4] = componentsInChildren[j];
				num4++;
			}
		}
	}

	public void ClearRpcMonoBehaviourCache()
	{
		if (RpcMonoBehaviours != null)
		{
			int num = RpcMonoBehaviours.Length;
			for (int i = 0; i < num; i++)
			{
				RpcMonoBehaviours[i] = null;
			}
			RpcMonoBehaviours = null;
		}
	}
	...
	public override string ToString()
	{
		...
		{
			Debug.LogWarningFormat("PhotonView: [null] -- viewID={0}", viewID);
		}
		if (this != null)
		{
			return string.Format("View ({3}){0} on {1} {2}", viewID, (!(base.gameObject != null)) ? "GO==null" : ((base.gameObject.name == null) ? "GO.name == null" : base.gameObject.name), (!isSceneView) ? string.Empty : "(scene)", prefix);
		}
		return "null";
	}
}

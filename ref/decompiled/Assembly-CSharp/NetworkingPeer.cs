...
internal class NetworkingPeer : LoadBalancingPeer, IPhotonPeerListener
{
	...
	public const string DefaultNameServerHost = "ns.exitgames.com";

	public const string ChinaNameServerHost = "ns-cn.exitgames.com";
	...
	protected internal List<PhotonView> photonViewList_ForSerialize = new List<PhotonView>();
	...
	public static bool m_bDiscardSerializeOnViews = true;
	...
	protected internal string AppVersion => string.Format("{0}_{1}", PhotonNetwork.gameVersion, "1.81b1");
	...
	public string NameServerHost
	{
		get
		{
			if (CloudRegion == CloudRegionCode.cn)
			{
				return "ns-cn.exitgames.com";
			}
	...
	private string GetNameServerAddress()
	{
		...
		return $"{arg}{NameServerHost}:{value}";
	}
	...
	public bool ConnectToNameServer()
	{
		...
		Debug.Log("PHOTON: connecting to " + NameServerAddress);
	...
	public override bool OpRaiseEvent(byte eventCode, object customEventContent, bool sendReliable, RaiseEventOptions raiseEventOptions)
	{
		...
		bool result = base.OpRaiseEvent(eventCode, customEventContent, sendReliable, raiseEventOptions);
		switch (eventCode)
		{
		case 200:
			T17NetPhotonTrafficStats.CaptureRPCStats(inbound: false, customEventContent, base.ByteCountLastOperation);
			break;
		case 201:
		case 206:
			T17NetPhotonTrafficStats.CaptureSerializeStats(inbound: false, customEventContent, base.ByteCountLastOperation);
			break;
		}
		return result;
	}
	...
	public void OnStatusChanged(StatusCode statusCode)
	{
		...
		case StatusCode.Connect:
			if (State == ClientState.ConnectingToNameServer)
			{
				if (PhotonNetwork.logLevel >= PhotonLogLevel.Full)
				{
					Debug.Log("PhotonNetworking: Connected to NameServer: " + base.ServerAddress);
				}
	...
	protected internal void ExecuteRpc(ExitGames.Client.Photon.Hashtable rpcData, PhotonPlayer sender)
	{
		...
		for (int j = 0; j < photonView.RpcMonoBehaviours.Length; j++)
		{
			...
			{
				Debug.LogError("ERROR You have missing MonoBehaviours on your gameobjects! (" + photonView.transform.name + ")");
				continue;
			}
			...
			for (int k = 0; k < value.Count; k++)
			{
				...
				if (cachedParemeters.Length == array2.Length)
				{
					...
					{
						num6++;
						if (PhotonNetwork.PhotonRpcTTY)
						{
							SendMonoMessage(PhotonNetworkingMessage.OnExecuteRPC, num, text, array);
						}
	...
	public bool LocalCleanPhotonView(PhotonView view)
	{
		...
		photonViewList_ForSerialize.Remove(view);
		view.ClearRpcMonoBehaviourCache();
		return photonViewList.Remove(view.viewID);
	}

	public PhotonView GetPhotonViewInDictionary(int viewID)
	{
		PhotonView value = null;
	...
	public void RegisterPhotonView_ToSerialize(PhotonView netView)
	{
		if (null != netView && netView.synchronization != 0 && photonViewList_ForSerialize.IndexOf(netView) == -1)
		{
			photonViewList_ForSerialize.Add(netView);
		}
	}

	public void UnregisterPhotonView_ToSerialize(PhotonView netView)
	{
		photonViewList_ForSerialize.Remove(netView);
	}
	...
	public void RegisterPhotonView(PhotonView netView)
	{
		...
		else
		{
			...
			T17NetManager.LogGoogleException("Registering a douplicate PhotonView! name : " + netView.name + " ID : " + netView.viewID);
		}
	}
	...
	public void NewSceneLoaded()
	{
		...
		foreach (KeyValuePair<int, PhotonView> photonView in photonViewList)
		{
			...
			if (value == null)
			{
				...
				photonViewList_ForSerialize.Remove(value);
			}
		}
	...
	public void RunViewUpdate()
	{
		...
		int num = 0;
		RaiseEventOptions raiseEventOptions = new RaiseEventOptions();
		for (int i = 0; i < photonViewList_ForSerialize.Count; i++)
		{
			PhotonView photonView = photonViewList_ForSerialize[i];
			if (photonView == null)
			{
				Debug.LogError("RunViewUpdate - null PhotonView detected");
				photonViewList_ForSerialize.Remove(photonView);
			}
			else
			{
				if (photonView.synchronization == ViewSynchronization.Off || !photonView.isMine || !photonView.gameObject.activeInHierarchy || blockSendingGroups.Contains(photonView.group))
				{
					continue;
				}
				object[] array = OnSerializeWrite(photonView);
				if (array == null)
				{
					continue;
				}
				if (photonView.synchronization == ViewSynchronization.ReliableDeltaCompressed || photonView.mixedModeIsReliable)
				{
					...
					if (!dataPerGroupReliable.TryGetValue(photonView.group, out value))
					{
						value = new ExitGames.Client.Photon.Hashtable(ObjectsInOneUpdate);
						dataPerGroupReliable[photonView.group] = value;
					}
				...
				if (!dataPerGroupUnreliable.TryGetValue(photonView.group, out value2))
				{
					value2 = new ExitGames.Client.Photon.Hashtable(ObjectsInOneUpdate);
					dataPerGroupUnreliable[photonView.group] = value2;
				}
	...
	private bool AlmostEquals(object one, object two)
	{
		...
		if (!one.Equals(two))
		{
			if (one is Vector3 target)
			{
				...
				return target.AlmostEquals(second, PhotonNetwork.precisionForVectorSynchronization);
			}
			if (one is Vector2 target2)
			{
				Vector2 second2 = (Vector2)two;
				return target2.AlmostEquals(second2, PhotonNetwork.precisionForVectorSynchronization);
			}
			if (one is Quaternion target3)
			{
				Quaternion second3 = (Quaternion)two;
				return target3.AlmostEquals(second3, PhotonNetwork.precisionForQuaternionSynchronization);
			}
			if (one is float target4)
			{
				float second4 = (float)two;
				return target4.AlmostEquals(second4, PhotonNetwork.precisionForFloatSynchronization);
			}
			if (one is byte b)
			{
				byte b2 = (byte)two;
			...
			if (one is double target5)
			{
				double second5 = (double)two;
				return target5.AlmostEquals(second5, PhotonNetwork.precisionForDoubleSynchronization);
			}
			if (one is ulong num3)
			{
				ulong num4 = (ulong)two;
				return num3 == num4;
			}
			Debug.Log("THIS DATA TYPE DOES NOT HAVE A EQUALS FUNCTION. THIS WILL ALWAYS BE RESENT : " + one.GetType().ToString());
			return false;
		}
		return true;
	}

...
[CreateAssetMenu(fileName = "Wall Block Functionality", menuName = "Team17/Items/Functionalities/Create Wall Block Functionality")]
public class WallBlockFunctionality : RepairFunctionality
{
	protected override bool FindTile(out FloorManager.Floor floor, out FloorManager.TileSystem_Type systemType, out int row, out int column)
	{
		if (m_Owner != null)
		{
			...
			if (targetTileRow != -1 && targetTileColumn != -1 && !currentFloor.IsVent())
			{
				...
				if ((damagableTile == null || !damagableTile.HasBeenDamaged()) && FloorManager.GetInstance().CheckDamagableTileExists(currentFloor, tileSystem_Type, targetTileRow, targetTileColumn, bIncludeInactive: true, out var action, out var health) && action == DamagableTile.DamageAction.Chip && health <= m_fMinRequiredHealth)
				{
					floor = currentFloor;
					row = targetTileRow;

...
public class GoogleAnalyticsMPV3
{
	...
	private void SendGaHitWithMeasurementProtocol(string url)
	{
		...
		{
			WWW request = new WWW(message);
			GoogleAnalyticsV3.getInstance().StartCoroutine(HandleWWW(request));
		}
	}
	...
	public IEnumerator HandleWWW(WWW request)
	{
		while (!request.isDone)
		{
			...
			if (request.responseHeaders.ContainsKey("STATUS"))
			{
				...
				{
					Debug.Log("Google Analytics hit request rejected with status code " + request.responseHeaders["STATUS"]);
				}
			}
			else if (GoogleAnalyticsV3.belowThreshold(logLevel, GoogleAnalyticsV3.DebugMode.WARNING))
			{
				Debug.LogErrorFormat("Google Analytics hit request failed with error {0} - URL = {1}", request.error, request.url);
			}
		}
		request.Dispose();
	}
	...
	public void LogException(ExceptionHitBuilder builder, string userData)
	{
		...
		SendGaHitWithMeasurementProtocol(url + AddRequiredMPParameter(Fields.HIT_TYPE, "exception") + AddOptionalMPParameter(Fields.EX_DESCRIPTION, builder.GetExceptionDescription()) + AddOptionalMPParameter(Fields.EX_FATAL, builder.IsFatal()) + AddCustomVariables(builder) + AddCampaignParameters(builder) + AddRequiredMPParameter(Fields.SCREEN_NAME, userData) + AddTrackerVals());
	}

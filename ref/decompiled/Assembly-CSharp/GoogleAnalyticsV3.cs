...
public class GoogleAnalyticsV3 : MonoBehaviour
{
	...
	public static bool bHandleExceptionOverrideUserData;

	public static string handleExceptionOverrideUserData = string.Empty;

	private void Start()
	{
		if (sendLaunchEvent)
		{
			Debug.Log("GoogleAnalyticsV3,Start: Google Analytics Auto Instrumentation Game Launch");
			LogEvent("Google Analytics", "Auto Instrumentation", "Game Launch", 0L);
		}
		if (UncaughtExceptionReporting)
		{
			Debug.Log("GoogleAnalyticsV3.Awake: Starting ExceptionReporting.");
			if (belowThreshold(logLevel, DebugMode.VERBOSE))
			{
				Debug.Log("GoogleAnalyticsV3.Awake : Enabling uncaught exception reporting.");
			}
		}
	}
	...
	private void InitializeTracker()
	{
		if (!initialized)
		{
			...
			Debug.Log("GoogleAnalyticsV3:Initializing Google Analytics 0.1.");
			...
			Debug.LogFormat("GoogleAnalyticsV3:Initializing Google Analytics -- Ver = {0}", bundleVersion);
			initialized = true;
	...
	public static void LogCommericalAnalyticEvent(string eventCategory, string eventAction, string eventLabel = "", long value = 0L)
	{
		if (instance != null)
		{
			instance.LogEvent(eventCategory, eventAction, eventLabel, value);
		}
	}
}

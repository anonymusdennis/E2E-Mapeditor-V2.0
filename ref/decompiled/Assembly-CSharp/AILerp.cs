...
public class AILerp : MonoBehaviour
{
	...
	{
		if (UpdateManager.time - lastRepath >= repathRate && canSearchAgain && canSearch && target != null)
		{
			SearchPath();
			return repathRate;
		}
		float num = repathRate - (UpdateManager.time - lastRepath);
	...
	protected virtual void ConfigurePathSwitchInterpolation()
	{
		...
		if (flag && !flag2)
		{
			...
			previousMovementStartTime = UpdateManager.time;
		}
		else
		{
	...
	protected virtual void Update()
	{
		...
		if (enableRotation && direction != Vector3.zero)
		{
			if (rotationIn2D)
			{
				...
				eulerAngles.z = Mathf.LerpAngle(eulerAngles.z, b, UpdateManager.deltaTime * rotationSpeed);
			...
			else
			{
				...
				tr.rotation = Quaternion.Slerp(rotation, b2, UpdateManager.deltaTime * rotationSpeed);
			}
		}
	...
	protected virtual Vector3 CalculateNextPosition(out Vector3 direction)
	{
		...
		distanceAlongSegment += UpdateManager.deltaTime * speed;
		...
		if (interpolatePathSwitches)
		{
			Vector3 a = previousMovementOrigin + Vector3.ClampMagnitude(previousMovementDirection, speed * (UpdateManager.time - previousMovementStartTime));
			return Vector3.Lerp(a, vector3, switchPathInterpolationSpeed * (UpdateManager.time - previousMovementStartTime));
		}
		return vector3;
	}
}

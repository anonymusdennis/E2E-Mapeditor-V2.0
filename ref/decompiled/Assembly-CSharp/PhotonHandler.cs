...
internal class PhotonHandler : MonoBehaviour
{
	...
	private const string PlayerPrefsKeyIsSet = "PUNCloudBestRegionSet";
	...
	internal static CloudRegionCode BestRegionCodeInPreferences
	{
		get
		{
			...
			bool flag = true;
			if (!string.IsNullOrEmpty(@string) && flag)
			{
		...
		set
		{
			if (value == CloudRegionCode.none)
			{
				...
				PlayerPrefs.DeleteKey("PUNCloudBestRegionSet");
			}
			else
			{
				PlayerPrefs.SetString("PUNCloudBestRegion", value.ToString());
				PlayerPrefs.SetInt("PUNCloudBestRegionSet", 1);
			}
		}
	}

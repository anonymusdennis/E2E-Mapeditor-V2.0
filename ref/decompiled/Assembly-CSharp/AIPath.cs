...
public class AIPath : MonoBehaviour
{
	...
	public virtual void OnPathComplete(Path _p)
	{
		...
		{
			Vector3 vector = ((!(UpdateManager.time - lastFoundWaypointTime < 0.3f)) ? aBPath.originalStartPoint : lastFoundWaypointPosition);
	...
	public virtual void Update()
	{
		if (canMove)
		{
			...
			{
				tr.Translate(vector * UpdateManager.deltaTime, Space.World);
			}
		}
	}
	...
	protected Vector3 CalculateVelocity(Vector3 currentPosition)
	{
		...
		while (currentWaypointIndex < vectorPath.Count - 1)
		{
			...
			if (num < pickNextWaypointDist * pickNextWaypointDist)
			{
				...
				lastFoundWaypointTime = UpdateManager.time;
		...
		{
			num3 = Mathf.Clamp(num3, 0f, magnitude / (UpdateManager.deltaTime * 2f));
		}
		return forward * num3;
	}
	...
	protected virtual void RotateTowards(Vector3 dir)
	{
		if (!(dir == Vector3.zero))
		{
			...
			Vector3 eulerAngles = Quaternion.Slerp(rotation, b, turningSpeed * UpdateManager.deltaTime).eulerAngles;
			eulerAngles.z = 0f;
			eulerAngles.x = 0f;

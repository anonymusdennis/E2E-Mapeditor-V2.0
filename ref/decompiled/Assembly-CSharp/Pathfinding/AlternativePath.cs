...
public class AlternativePath : MonoModifier
{
	...
	private OnPathDelegate m_applyNowDel;

	private OnPathDelegate m_clearOnDestroyDel;
	...
	{
		m_applyNowDel = ApplyNow;
		m_clearOnDestroyDel = ClearOnDestroy;
		base.Awake();
	}
	...
	public override void Apply(Path p)
	{
		...
		lock (lockObject)
		{
			...
			if (!waitingForApply)
			{
				...
				AstarPath.OnPathPreSearch = (OnPathDelegate)Delegate.Combine(AstarPath.OnPathPreSearch, m_applyNowDel);
			}
		}
	}
	...
	public new void OnDestroy()
	{
		...
		lock (lockObject)
		{
			...
			{
				waitingForApply = true;
				AstarPath.OnPathPreSearch = (OnPathDelegate)Delegate.Combine(AstarPath.OnPathPreSearch, m_clearOnDestroyDel);
			}
		}
	...
	private void ClearOnDestroy(Path p)
	{
		lock (lockObject)
		{
			AstarPath.OnPathPreSearch = (OnPathDelegate)Delegate.Remove(AstarPath.OnPathPreSearch, m_clearOnDestroyDel);
	...
	private void ApplyNow(Path somePath)
	{
		...
		{
			waitingForApply = false;
			AstarPath.OnPathPreSearch = (OnPathDelegate)Delegate.Remove(AstarPath.OnPathPreSearch, m_applyNowDel);
			InversePrevious();
			if (destroyed)
			{
				return;

...
public class RichAI : MonoBehaviour
{
	...
	private IEnumerator SearchPaths()
	{
		while (true)
		{
			if (!repeatedlySearchPaths || waitingForPathCalc || !canSearchPath || UpdateManager.time - lastRepath < repathRate)
			{
				yield return null;
				continue;
			}
	...
	{
		deltaTime = Mathf.Min(Time.smoothDeltaTime * 2f, UpdateManager.deltaTime);
		if (rp != null)
		{
			...
			if (currentPart is RichFunnel richFunnel)
			{
				...
				if (UpdateManager.frameCount % 5 == 0 && wallForce > 0f && wallDist > 0f)
				{
					wallBuffer.Clear();

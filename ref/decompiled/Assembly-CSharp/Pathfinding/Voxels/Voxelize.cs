...
public class Voxelize
{
	...
	public void BuildPolyMesh(VoxelContourSet cset, int nvp, out VoxelMesh mesh)
	{
		...
		for (int j = 0; j < cset.conts.Count; j++)
		{
			...
			if (voxelContour.nverts >= 3)
			{
				...
				{
					ref Int3 reference = ref array[num4];
					reference = new Int3(voxelContour.verts[m * 4], voxelContour.verts[m * 4 + 1], voxelContour.verts[m * 4 + 2]);
					num4++;
				}
			}
		}
		...
		{
			ref Int3 reference2 = ref array3[n];
			reference2 = array[n];
		}
	...
	public void VoxelizeInput()
	{
		...
		for (int j = 0; j < inputExtraMeshes.Count; j++)
		{
			...
			{
				ref Vector3 reference = ref array6[k];
				reference = matrix.MultiplyPoint3x4(vertices[k]);
			}
	...
	public void BuildCompactField()
	{
		...
		while (num5 < num4)
		{
			for (int i = 0; i < num2; i++)
			{
				...
				{
					ref CompactVoxelCell reference = ref voxelArea.compactCells[i + num5];
					reference = new CompactVoxelCell(0u, 0u);
					continue;
				}
				uint i2 = num;
				uint num8 = 0u;
				while (num7 != -1)
				{
					if (linkedSpans[num7].area != 0)
					{
						...
						ref CompactVoxelSpan reference2 = ref voxelArea.compactSpans[num];
						reference2 = new CompactVoxelSpan((ushort)((top <= 65535) ? ((uint)top) : 65535u), (num9 - top <= 65535) ? ((uint)(num9 - top)) : 65535u);
				...
				ref CompactVoxelCell reference3 = ref voxelArea.compactCells[i + num5];
				reference3 = new CompactVoxelCell(i2, num8);
			}
			num5 += num2;
			num6++;
		}

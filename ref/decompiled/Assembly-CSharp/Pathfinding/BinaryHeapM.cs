...
public class BinaryHeapM
{
	...
	public void Add(PathNode node)
	{
		...
		if (numberOfItems == binaryHeap.Length)
		{
			...
			{
				ref Tuple reference = ref array[i];
				reference = binaryHeap[i];
			}
			binaryHeap = array;
		}
		...
		while (num2 != 0)
		{
			...
			{
				ref Tuple reference2 = ref binaryHeap[num2];
				reference2 = binaryHeap[num3];
	...
	public PathNode Remove()
	{
		...
		ref Tuple reference = ref binaryHeap[0];
		reference = binaryHeap[numberOfItems];
		int num = 0;
		while (true)
		{
			int num2 = num;
	...
	public void Rebuild()
	{
		for (int i = 2; i < numberOfItems; i++)
		{
			...
			while (num != 1)
			{
				...
				{
					ref Tuple reference = ref binaryHeap[num];
					reference = binaryHeap[num2];

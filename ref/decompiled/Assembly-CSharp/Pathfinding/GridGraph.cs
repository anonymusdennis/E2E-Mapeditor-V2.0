...
public class GridGraph : NavGraph, IUpdatableGraph, IRaycastableGraph
{
	...
	public virtual void UpdateNodePositionCollision(GridNode node, int x, int z, bool resetPenalty = true)
	{
		...
		else if (m_bIsUndergroundLayer)
		{
			if (m_levelScript == null)
			{
				m_levelScript = LevelScript.GetInstance();
			}
			if (!node.Walkable || m_levelScript == null || !m_levelScript.hasWalkableUnderground)
			{
				node.Tag = 2u;
				node.Penalty = 10u;
			}
	...
	public virtual void CalculateConnections(int x, int z, GridNode node)
	{
			...
			if (neighbours == NumNeighbours.Eight)
			{
				...
				else
				{
					for (int k = 0; k < 4; k++)
					{
						...
						{
							GridNode gridNode = nodes[nodeInGridIndex + neighbourOffsets[k + 4]];
							if (!node.m_bHasDamagableTile && !gridNode.m_bHasDamagableTile && IsValidConnection(node, gridNode))
							{
								num4 |= 1 << k + 4;
							}
						}
		...
		for (int l = 0; l < hexagonNeighbourIndices.Length; l++)
		{
			...
			{
				GridNode n3 = nodes[nodeInGridIndex + neighbourOffsets[num8]];
				node.SetConnectionInternal(num8, IsValidConnection(node, n3));
			}
		}
	}
	...
	public void UpdateArea(GraphUpdateObject o)
	{
		...
		GetBoundsMinMax(bounds, inverseMatrix, out var min, out var max);
		int xmin = Mathf.RoundToInt(min.x - 0.5f);
		int xmax = Mathf.RoundToInt(max.x - 0.5f);
		int ymin = Mathf.RoundToInt(min.z - 0.5f);
		int ymax = Mathf.RoundToInt(max.z - 0.5f);
	...
	public bool Linecast(Vector3 _a, Vector3 _b, GraphNode hint, out GraphHitInfo hit, List<GraphNode> trace)
	{
		...
		GridNodeBase gridNodeBase = GetNearest(matrix.MultiplyPoint3x4(outA), AstarPath.m_NNnone).node as GridNodeBase;
		GridNodeBase gridNodeBase2 = GetNearest(matrix.MultiplyPoint3x4(outB), AstarPath.m_NNnone).node as GridNodeBase;
		...
		while (gridNodeBase3.NodeInGridIndex != gridNodeBase2.NodeInGridIndex)
		{
			...
			GridNodeBase gridNodeBase4 = GetNeighbourAlongDirection(gridNodeBase3, num6);
			if (gridNodeBase3.Tag != 0)
			{
				gridNodeBase4 = null;
			}
			if (gridNodeBase4 != null)
			{
	...
	{
		return Linecast((Vector3)GetNearest(a, AstarPath.m_NNnone).node.position, (Vector3)GetNearest(b, AstarPath.m_NNnone).node.position, hint, out hit);
	}

...
public class RecastGraph : NavGraph, INavmesh, IRaycastableGraph, IUpdatableGraph, INavmeshHolder
{
		...
		if (tiles != null)
		{
			...
			for (int i = 0; i < tiles.Length; i++)
			{
				...
				if (navmeshTile != null)
				{
					...
					{
						ref Int3 reference = ref verts[j];
						reference = (Int3)matrix4x.MultiplyPoint((Vector3)verts[j]);
					}
	...
	void IUpdatableGraph.UpdateArea(GraphUpdateObject guo)
	{
		...
		for (int num2 = touchingTiles.ymin; num2 <= touchingTiles.ymax; num2++)
		{
			...
			{
				NavmeshTile navmeshTile = tiles[num3 + num2 * tileXCount];
	...
	public void RemoveConnectionsFromTo(NavmeshTile a, NavmeshTile b)
	{
		...
		for (int i = 0; i < a.nodes.Length; i++)
		{
			...
			{
				if (triangleMeshNode.connections[j] is TriangleMeshNode triangleMeshNode2)
				{
	...
	public override void ScanInternal(OnScanStatus statusCallback, bool bUpdateNetworkService = false)
	{
	...
	protected void BuildTileMesh(Voxelize vox, int x, int z)
	{
		...
		{
			ref Int3 reference = ref mesh.verts[i];
			reference = mesh.verts[i] * 1000 * vox.cellScale + (Int3)vox.voxelOffset;
		}
	...
	private NavmeshTile CreateTile(Voxelize vox, VoxelMesh mesh, int x, int z)
	{
		...
		for (int i = 0; i < navmeshTile.verts.Length; i++)
		{
			if (!dictionary.ContainsKey(navmeshTile.verts[i]))
			{
				...
				array[i] = num;
				ref Int3 reference = ref navmeshTile.verts[num];
				reference = navmeshTile.verts[i];
				num++;
			}
			else
			{
		...
		for (int k = 0; k < num; k++)
		{
			ref Int3 reference2 = ref array2[k];
			reference2 = navmeshTile.verts[k];
		}
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			for (int j = 0; j < num2; j++)
			{
				...
				for (int k = 0; k < num; k++)
				{
					...
					ref Vector3 reference = ref array2[j * num + k];
					reference = new Vector3((float)num6 * heightmapScale.x, heights[num5, num6] * y, (float)num5 * heightmapScale.z) + position;
		...
		if (col is SphereCollider || col is CapsuleCollider)
		{
			...
			if (capsuleCache == null)
			{
				...
				for (int j = 0; j < num3; j++)
				{
					for (int k = 0; k < num4; k++)
					{
						ref Vector3 reference2 = ref array[k + j * num4];
						reference2 = new Vector3(Mathf.Cos((float)k * (float)Math.PI * 2f / (float)num4) * Mathf.Sin((float)j * (float)Math.PI / (float)(num3 - 1)), Mathf.Cos((float)j * (float)Math.PI / (float)(num3 - 1)) + ((j >= num3 / 2) ? (0f - num2) : num2), Mathf.Sin((float)k * (float)Math.PI * 2f / (float)num4) * Mathf.Sin((float)j * (float)Math.PI / (float)(num3 - 1)));
					}
				}
				ref Vector3 reference3 = ref array[array.Length - 2];
	...
	{
		return Linecast(origin, end, GetNearest(origin, AstarPath.m_NNnone).node);
	}
	...
	public override void DeserializeExtraInfo(GraphSerializationContext ctx)
	{
		...
		for (int i = 0; i < tileZCount; i++)
		{
			for (int j = 0; j < tileXCount; j++)
			{
				...
				for (int l = 0; l < navmeshTile.verts.Length; l++)
				{
					ref Int3 reference = ref navmeshTile.verts[l];
					reference = ctx.DeserializeInt3();
				}

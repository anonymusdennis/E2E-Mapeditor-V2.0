...
public class NavmeshAdd : MonoBehaviour
{
	...
	public void RebuildMesh()
	{
		...
		reference = new Vector3((0f - rectangleSize.x) * 0.5f, 0f, (0f - rectangleSize.y) * 0.5f);
		ref Vector3 reference2 = ref verts[1];
		reference2 = new Vector3(rectangleSize.x * 0.5f, 0f, (0f - rectangleSize.y) * 0.5f);
		ref Vector3 reference3 = ref verts[2];
		reference3 = new Vector3(rectangleSize.x * 0.5f, 0f, rectangleSize.y * 0.5f);
		ref Vector3 reference4 = ref verts[3];
		reference4 = new Vector3((0f - rectangleSize.x) * 0.5f, 0f, rectangleSize.y * 0.5f);
	}
	...
	public void GetMesh(Int3 offset, ref Int3[] vbuffer, out int[] tbuffer)
	{
		...
		if (useRotation)
		{
			...
			{
				ref Int3 reference = ref vbuffer[i];
				reference = offset + (Int3)matrix4x.MultiplyPoint3x4(verts[i]);
			}
		}
		else
		{
			...
			{
				ref Int3 reference2 = ref vbuffer[j];
				reference2 = offset + (Int3)(vector + verts[j] * meshScale);
			}
		}
	}
}

...
public class Agent : IAgent
{
	...
	internal void CalculateVelocity(Simulator.WorkerContext context)
	{
		...
		float num3 = ((simulator.algorithm != Simulator.SamplingAlgorithm.GradientDescent) ? 5f : 1f);
		for (int i = 0; i < simulator.obstacles.Count; i++)
		{
			...
			do
			{
				...
				if (Mathf.Abs(num5) < neighbourDist)
				{
					...
					{
						ref VO reference = ref vos[num];
						reference = new VO(vector, new Vector2(obstacleVertex2.position.x, obstacleVertex2.position.z) - vector, obstacleVertex2.dir, num3 * 2f);
						num++;
					}
					else if (num5 > 0f)
					{
						...
						ref VO reference2 = ref vos[num];
						reference2 = new VO(vector, p, p2, normalized, normalized2, num3);
						num++;
					}
				}
				obstacleVertex2 = obstacleVertex2.next;
		...
		for (int j = 0; j < neighbours.Count; j++)
		{
			...
			if (!(num7 - num8 < 0f))
			{
				...
				ref VO reference3 = ref vos[num];
				reference3 = new VO(vector4, vector5, num9, sideChooser, num2, 1f);
				num++;
				if (DebugDraw)
				{
			...
			if (DebugDraw)
			{
				for (int k = 0; k < 40; k++)
				{
					for (int l = 0; l < 40; l++)
					{
						...
						for (int m = 0; m < num; m++)
						{
							zero2 += vos[m].Sample(vector6 - vector, out var num11);
							if (num11 > num10)
							{
								num10 = num11;
							}
		...
		else
		{
			...
			for (int n = 0; n < 8; n++)
			{
				ref Vector2 reference4 = ref samplePos[num12];
				reference4 = vector13 * Mathf.Sin((float)n * (float)Math.PI * 2f / 8f) + vector12 * (1f + Mathf.Cos((float)n * (float)Math.PI * 2f / 8f));
			...
			{
				ref Vector2 reference5 = ref samplePos[num12];
				reference5 = vector13 * Mathf.Cos(((float)num14 + 0.5f) * (float)Math.PI * 2f / 6f) + vector12 * (1.6666666f + Mathf.Sin(((float)num14 + 0.5f) * (float)Math.PI * 2f / 6f));
				sampleSize[num12] = num13 * 0.3f;
				num12++;
			}
			for (int num15 = 0; num15 < 6; num15++)
			{
				ref Vector2 reference6 = ref samplePos[num12];
				reference6 = vector2 + new Vector2(num13 * 0.2f * Mathf.Cos(((float)num15 + 0.5f) * (float)Math.PI * 2f / 6f), num13 * 0.2f * Mathf.Sin(((float)num15 + 0.5f) * (float)Math.PI * 2f / 6f));
				sampleSize[num12] = num13 * 0.2f * 2f;
				num12++;
			}
			ref Vector2 reference7 = ref samplePos[num12];
			reference7 = vector2 * 0.5f;
			...
			for (int num18 = 0; num18 < 3; num18++)
			{
					...
					if (num22 < bestScores[0])
					{
						for (int num23 = 0; num23 < 3; num23++)
						{
							if (num22 >= bestScores[num23 + 1])
							{
								...
								ref Vector2 reference8 = ref bestPos[num23];
								reference8 = samplePos[num19];
								break;
							}
						}
					}
				...
				for (int num24 = 0; num24 < 3; num24++)
				{
					...
					ref Vector2 reference9 = ref samplePos[num12];
					reference9 = vector15 + new Vector2(num26, num26);
					ref Vector2 reference10 = ref samplePos[num12 + 1];
					reference10 = vector15 + new Vector2(0f - num26, num26);
					ref Vector2 reference11 = ref samplePos[num12 + 2];
					reference11 = vector15 + new Vector2(0f - num26, 0f - num26);
					ref Vector2 reference12 = ref samplePos[num12 + 3];
					reference12 = vector15 + new Vector2(num26, 0f - num26);

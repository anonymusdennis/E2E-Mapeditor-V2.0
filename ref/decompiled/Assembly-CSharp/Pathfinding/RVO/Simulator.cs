...
public class Simulator
{
	...
	public void Update()
	{
		if (lastStep < 0f)
		{
			lastStep = UpdateManager.time;
			deltaTime = DesiredDeltaTime;
			prevDeltaTime = deltaTime;
			lastStepInterpolationReference = lastStep;
		}
		if (UpdateManager.time - lastStep >= DesiredDeltaTime)
		{
			for (int i = 0; i < agents.Count; i++)
			{
				agents[i].Interpolate((UpdateManager.time - lastStepInterpolationReference) / DeltaTime);
			}
			lastStepInterpolationReference = UpdateManager.time;
			prevDeltaTime = DeltaTime;
			deltaTime = UpdateManager.time - lastStep;
			lastStep = UpdateManager.time;
		...
		if (Interpolation)
		{
			...
			{
				agents[num9].Interpolate((UpdateManager.time - lastStepInterpolationReference) / DeltaTime);
			}
		}
	}
}

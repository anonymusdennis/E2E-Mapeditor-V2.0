...
public class NavMeshGraph : NavGraph, INavmesh, IUpdatableGraph, INavmeshHolder, IRaycastableGraph
{
	...
	public static bool Linecast(INavmesh graph, Vector3 tmp_origin, Vector3 tmp_end, GraphNode hint, out GraphHitInfo hit, List<GraphNode> trace)
	{
		...
		{
			triangleMeshNode = (graph as NavGraph).GetNearest(tmp_origin, AstarPath.m_NNnone).node as TriangleMeshNode;
			if (triangleMeshNode == null)
			{
	...
	public override void ScanInternal(OnScanStatus statusCallback, bool bUpdateNetworkService = false)
	{
		if (!(sourceMesh == null))
		{
			...
			GenerateNodes(vectorVertices, triangles, out originalVertices, out _vertices, bUpdateNetworkService);
		}
	}

	private void GenerateNodes(Vector3[] vectorVertices, int[] triangles, out Vector3[] originalVertices, out Int3[] vertices, bool bUpdateNetworkService = false)
	{
	...
	public override void DeserializeExtraInfo(GraphSerializationContext ctx)
	{
		...
		for (int i = 0; i < num3; i++)
		{
			...
			reference2 = ctx.DeserializeVector3();
		}
		bbTree = new BBTree();

...
public class EditorLevelEditorManager : BaseLevelManager
{
	protected static EditorLevelEditorManager m_LevelEditorInstance;

	protected bool m_DeleteingRoom;

	public static EditorLevelEditorManager GetLevelEditorInstance()
	{
		return m_LevelEditorInstance;
	}

	protected override void Awake()
	{
		base.Awake();
		m_LevelEditorInstance = this;
	}

	protected override void OnDestroy()
	{
		if (m_LevelEditorInstance == this)
		{
			m_LevelEditorInstance = null;
		}
	}

	public override bool IsEverythingSetUp()
	{
		bool result = base.IsEverythingSetUp();
		for (int i = 0; i < 6; i++)
		{
	...
	public override void AddSingle(ref BuildingInstructionManager.InstructionOnceElement obj, bool bStorePrevious = true)
	{
		...
		{
			TileIDData previous = PlaceTile(obj.m_XPosition, obj.m_YPosition, (BuildingBlock_Tile)block, obj.m_iRandomSeed);
			if (bStorePrevious)
			{
		...
		case BaseBuildingBlock.BuildingBlockType.Object:
			obj.m_Previous = TileIDData.IDMask;
			PlaceObject(obj.m_XPosition, obj.m_YPosition, (BuildingBlock_Object)block);
			break;
	...
	public override void RemoveSingle(ref BuildingInstructionManager.InstructionOnceElement obj)
	{
		BaseBuildingBlock block = BuildingBlockManager.GetBlock(obj.m_BuildingBlockID);
		if (!(block == null))
		{
			...
			{
			case BaseBuildingBlock.BuildingBlockType.Tile:
				ReplaceTile(obj.m_XPosition, obj.m_YPosition, obj.m_Previous);
				break;
			case BaseBuildingBlock.BuildingBlockType.Decoration:
			case BaseBuildingBlock.BuildingBlockType.Object:
				RemoveObject(obj.m_XPosition, obj.m_YPosition, (BuildingBlock_Object)block);
				break;
			case BaseBuildingBlock.BuildingBlockType.Wall:
				break;
			}
	...
	public override void AddSingleWall(ref BuildingInstructionManager.InstructionOnceWallElement obj, bool bStorePrevious = true)
	{
		...
		{
			TileIDData previous = PlaceWall(obj.m_XPosition, obj.m_YPosition, (BuildingBlock_Wall)block, obj.m_iRandomSeed);
			if (bStorePrevious)
			{
				obj.m_Previous = previous;
			}
			...
			if (floorTileID != -1)
			{
				...
				previous = ((!(block2 != null)) ? ValidatePrevious(m_BuildingLayers[(uint)m_CurrentLayer].m_TileTileIDs[obj.m_YPosition * 120 + obj.m_XPosition]) : PlaceTile(obj.m_XPosition, obj.m_YPosition, (BuildingBlock_Tile)block2, obj.m_iRandomSeed, bMarkAsChanged: false));
			}
			else
			{
				previous = ValidatePrevious(m_BuildingLayers[(uint)m_CurrentLayer].m_TileTileIDs[obj.m_YPosition * 120 + obj.m_XPosition]);
			}
			if (bStorePrevious)
			{
				obj.m_PreviousTile = previous;
			}
		}
	}

	public override void RemoveSingleWall(ref BuildingInstructionManager.InstructionOnceWallElement obj)
	{
		BaseBuildingBlock block = BuildingBlockManager.GetBlock(obj.m_BuildingBlockID);
		if (block == null)
		{
		...
		if (blockType == BaseBuildingBlock.BuildingBlockType.Wall)
		{
			ReplaceWall(obj.m_XPosition, obj.m_YPosition, obj.m_Previous);
			int floorTileID = ((BuildingBlock_Wall)block).m_FloorTileID;
			if (floorTileID != -1)
			{
				ReplaceTile(obj.m_XPosition, obj.m_YPosition, obj.m_PreviousTile);
			}
		}
	}
	...
	public override void AddArea(ref BuildingInstructionManager.InstructionAreaElement obj, bool bStorePrevious = true)
	{
		...
		bool ignoreChecks = BuildingInstructionManager.GetInstance().m_IgnoreChecks;
		...
		{
			flag3 = (buildingBlock_Tile.m_ValidLayers & 0x555) != 0;
			flag = buildingBlock_Tile.m_BlockingTile;
			flag2 = buildingBlock_Tile.m_NoBlockingBelow;
		}
		...
		int num9 = 0;
		if (m_CurrentLayer != LevelLayers.Roof && (!flag3 || buildingBlock_Tile == null))
		{
			num9 = (flag4 ? 1 : 2);
		}
		bool flag5 = true;
			...
			for (int j = num; j < num2; j++)
			{
				...
				for (int k = 0; k < num10; k++)
				{
					...
					if (!ignoreChecks)
					{
						if (num8 != 0)
						{
							flag5 = (m_BuildingLayers[(int)m_CurrentLayer - num8].m_TileProperties[num11] & TileProperty.TileExistsMask) == TileProperty.TileExistsMask;
						}
						if (num9 != 0 && flag5)
						{
							for (int l = 1; l <= num9; l++)
							{
								...
								flag5 = (m_BuildingLayers[(int)m_CurrentLayer + l].m_TileProperties[num11] & TileProperty.TileMask) != TileProperty.TileMask;
							}
						}
					}
					if (!flag5)
					...
					else if (flag && k == 0 && (tileProperty2 & TileProperty.NoBlockingMask) == TileProperty.NoBlockingMask)
					{
						flag5 = false;
					}
					if (flag2 && (int)m_CurrentLayer > 1 && k == 0)
					{
						TileProperty tileProperty3 = m_BuildingLayers[(uint)(m_CurrentLayer - 1)].m_TileProperties[num11];
				...
				TileIDData tileIDData = ((!flag5) ? (~TileIDData.ComplexBits) : PlaceTile(j, i, buildingBlock_Tile, random.Next()));
				num6++;
				if (bStorePrevious)
				{
					obj.m_Previous[num5] = tileIDData;
				}
				num5++;
			}
			num6 += num7;
	...
	public override void RemoveArea(ref BuildingInstructionManager.InstructionAreaElement obj)
	{
		BuildingBlock_Tile buildingBlock_Tile = BuildingBlockManager.GetBlock(obj.m_BuildingBlockID) as BuildingBlock_Tile;
		if ((buildingBlock_Tile == null && obj.m_BuildingBlockID != -1) || (!(buildingBlock_Tile == null) && buildingBlock_Tile.BlockType != BaseBuildingBlock.BuildingBlockType.Tile))
		{
			return;
		}
		int num = obj.m_XPosition;
		...
		for (int i = num3; i < num4; i++)
		{
			for (int j = num; j < num2; j++)
			{
				ReplaceTile(j, i, obj.m_Previous[num5]);
				num5++;
			}
		}
	}
		...
		for (int i = num3; i < num4; i++)
		{
			for (int j = num; j < num2; j++)
			{
				...
				if (!ignoreChecks)
				{
					if (num8 != 0)
					{
						flag2 = (m_BuildingLayers[(int)m_CurrentLayer - num8].m_TileProperties[num6] & TileProperty.TileExistsMask) == TileProperty.TileExistsMask;
					}
					if (num9 != 0 && flag2)
					{
						for (int k = 1; k <= num9; k++)
						{
							...
							flag2 = (m_BuildingLayers[(int)m_CurrentLayer + k].m_TileProperties[num6] & TileProperty.TileMask) != TileProperty.TileMask;
						}
					}
				...
				{
					TileIDData tileIDData = PlaceWall(j, i, buildingBlock_Wall, random.Next());
					if (bStorePrevious)
					{
						obj.m_Previous[num5] = tileIDData;
					}
					if (buildingBlock_Tile != null)
					{
						tileIDData = PlaceTile(j, i, buildingBlock_Tile, random.Next(), bMarkAsChanged: false);
					}
					else if (bStorePrevious)
					{
						tileIDData = ValidatePrevious(m_BuildingLayers[(uint)m_CurrentLayer].m_TileTileIDs[i * 120 + j]);
					}
					if (bStorePrevious)
					{
						obj.m_PreviousTile[num5] = tileIDData;
					}
				}
				else if (bStorePrevious)
				{
					obj.m_Previous[num5] = ~TileIDData.ComplexBits;
					obj.m_PreviousTile[num5] = ~TileIDData.ComplexBits;
				}
				num5++;
				num6++;
			}
	...
	public override void CreateZone(ref BuildingInstructionManager.InstructionZoneElement obj)
	{
		...
		{
			obj.m_iID = m_ZoneManager.CreateAZone(obj.m_ZoneType, m_CurrentLayer, obj.m_Left, obj.m_Bottom, obj.m_Width, obj.m_Height, obj.m_ZonePrint, obj.m_iID);
		}
	}
	...
	public override void AddToZone(ref BuildingInstructionManager.InstructionZoneElement obj)
	{
		if (!(m_ZoneManager == null) || !((m_ZoneManager = LevelEditor_ZoneManager.GetInstance()) == null))
		{
			m_ZoneManager.AddToZone(obj.m_iID, obj.m_Left, obj.m_Bottom, obj.m_Width, obj.m_Height, ref obj.m_ZonePrint);
		}
	}
	...
	public override void SubtractFromZone(ref BuildingInstructionManager.InstructionZoneElement obj)
	{
		if (!(m_ZoneManager == null) || !((m_ZoneManager = LevelEditor_ZoneManager.GetInstance()) == null))
		{
			m_ZoneManager.SubtractFromZone(obj.m_iID, obj.m_Left, obj.m_Bottom, obj.m_Width, obj.m_Height, ref obj.m_ZonePrint);
		}
	}

	public override void RemoveAreaWall(ref BuildingInstructionManager.InstructionAreaWallElement obj)
	{
		BuildingBlock_Wall buildingBlock_Wall = BuildingBlockManager.GetBlock(obj.m_BuildingBlockID) as BuildingBlock_Wall;
		if (buildingBlock_Wall == null || buildingBlock_Wall.BlockType != BaseBuildingBlock.BuildingBlockType.Wall)
		{
		...
		for (int i = num3; i < num4; i++)
		{
			for (int j = num; j < num2; j++)
			{
				ReplaceWall(j, i, obj.m_Previous[num5]);
				int floorTileID = buildingBlock_Wall.m_FloorTileID;
				if (floorTileID != -1)
				{
					ReplaceTile(j, i, obj.m_PreviousTile[num5]);
				}
				num5++;
			}
		}
	...
	public override void AddCommand(ref BuildingInstructionManager.InstructionCommandElement obj, bool bStorePrevious = true)
	{
		...
		case BuildingInstructionManager.CommandsEnum.Dec_Layer:
			m_CurrentLayer--;
			break;
		case BuildingInstructionManager.CommandsEnum.Start_Undo:
			m_UndoCount++;
			break;
		case BuildingInstructionManager.CommandsEnum.End_Undo:
			m_UndoCount--;
			break;
	...
	public override void RemoveCommand(ref BuildingInstructionManager.InstructionCommandElement obj)
	{
		switch (obj.m_Commmand)
		{
		case BuildingInstructionManager.CommandsEnum.Set_Layer:
			m_CurrentLayer = (LevelLayers)obj.m_PreviousValue;
			break;
		case BuildingInstructionManager.CommandsEnum.Start_Undo:
			m_UndoCount--;
			break;
		...
		case BuildingInstructionManager.CommandsEnum.Dec_Layer:
			...
			break;
		case BuildingInstructionManager.CommandsEnum.Set_Environment:
			m_CurrentEnvironment = (LayersEnvironment)obj.m_PreviousValue;
			break;
		case BuildingInstructionManager.CommandsEnum.Start_Room:
			CancelComplexObjectAllocation(m_CurrentComplexAllocation);
			break;
		case BuildingInstructionManager.CommandsEnum.End_Room:
			SetComplexObjectAllocation(obj.m_PreviousValue);
			break;
		case BuildingInstructionManager.CommandsEnum.Start_DeleteRoom:
			EndComplexObjectAllocation();
			break;
		case BuildingInstructionManager.CommandsEnum.End_DeleteRoom:
		{
			int value2 = obj.m_Value;
			StartComplexObjectAllocation(obj.m_PreviousValue);
			BuildingInstructionManager.GetInstance().SetCommandPreviousValue(value2, m_CurrentComplexAllocation);
			break;
		}
		case BuildingInstructionManager.CommandsEnum.Set_Inside:
		case BuildingInstructionManager.CommandsEnum.Set_Outside:
		{
			...
			for (int i = 0; i < num; i++)
			{
				for (int j = 0; j < num2; j++)
				{
					if (obj.m_Commmand == BuildingInstructionManager.CommandsEnum.Set_Inside)
					{
						TileProperty[] tileProperties;
						int num7;
						(tileProperties = m_BuildingLayers[(uint)m_CurrentLayer].m_TileProperties)[num7 = num5++] = tileProperties[num7] & TileProperty.InverseEnvironmentMask;
	...
	protected TileIDData PlaceTile(int X, int Y, BuildingBlock_Tile obj, int seed, bool bMarkAsChanged = true, bool bReplaceSame = false)
	{
		int num = Y * 120 + X;
		...
		TileIDData tileIDData3 = ValidatePrevious(tileIDData2);
		...
		if ((int)m_CurrentLayer > 1)
		{
			...
			{
				int num5;
				(tileProperties = m_BuildingLayers[(uint)(m_CurrentLayer - 1)].m_TileProperties)[num5 = num] = tileProperties[num5] | TileProperty.NoBlockingMask;
			}
			else
			{
				int num6;
				(tileProperties = m_BuildingLayers[(uint)(m_CurrentLayer - 1)].m_TileProperties)[num6 = num] = tileProperties[num6] & TileProperty.InverseNoBlockingMask;
			}
		}
		if (!m_DeleteingRoom)
		{
		...
		{
			flag = false;
			tileIDData3 = ~TileIDData.ComplexBits;
		}
		if (flag)
		{
	...
	private void ReplaceTile(int X, int Y, TileIDData previous)
	{
		if (previous == ~TileIDData.ComplexBits)
		{
			return;
		}
		int num = Y * 120 + X;
		...
		m_BuildingBlockManager.AdjustLimitationTotal(m_BuildingLayers[(uint)m_CurrentLayer].m_TileTileIDs[num], bAdd: false);
		m_BuildingLayers[(uint)m_CurrentLayer].m_TileTileIDs[num] = previous;
		m_BuildingBlockManager.AdjustLimitationTotal(previous, bAdd: true);
		if (m_CurrentComplexProcessed != 0)
		{
			BaseLevelManager.RemoveRoomNumberInProperty(ref m_BuildingLayers[(uint)m_CurrentLayer], num, (int)m_CurrentComplexProcessed);
		}
		int num2;
		TileProperty[] tileProperties;
		(tileProperties = m_BuildingLayers[(uint)m_CurrentLayer].m_TileProperties)[num2 = num] = tileProperties[num2] & TileProperty.InverseEnvironmentMask;
		int num3;
		(tileProperties = m_BuildingLayers[(uint)m_CurrentLayer].m_TileProperties)[num3 = num] = tileProperties[num3] & TileProperty.InverseTileBlockingMask;
		if ((previous & TileIDData.IDMask) == TileIDData.IDMask)
		{
			int num4;
			(tileProperties = m_BuildingLayers[(uint)m_CurrentLayer].m_TileProperties)[num4 = num] = tileProperties[num4] & TileProperty.InverseTileMask;
			if ((int)m_CurrentLayer > 1)
			{
				int num5;
				(tileProperties = m_BuildingLayers[(uint)(m_CurrentLayer - 1)].m_TileProperties)[num5 = num] = tileProperties[num5] & TileProperty.InverseNoBlockingMask;
			}
		}
		else
		{
			int num6;
			(tileProperties = m_BuildingLayers[(uint)m_CurrentLayer].m_TileProperties)[num6 = num] = tileProperties[num6] | TileProperty.TileMask;
			BuildingBlock_Tile buildingBlock_Tile = BuildingBlockManager.GetBlock(previous) as BuildingBlock_Tile;
			if (buildingBlock_Tile != null)
			{
				if (((uint)buildingBlock_Tile.m_ValidLayers & 0x555u) != 0)
				{
					int num7;
					(tileProperties = m_BuildingLayers[(uint)m_CurrentLayer].m_TileProperties)[num7 = num] = tileProperties[num7] | TileProperty.EnvironmentMask;
				}
				if (buildingBlock_Tile.m_BlockingTile)
				{
					int num8;
					(tileProperties = m_BuildingLayers[(uint)m_CurrentLayer].m_TileProperties)[num8 = num] = tileProperties[num8] | TileProperty.TileBlockingMask;
				}
				if ((int)m_CurrentLayer > 1)
				{
					if (buildingBlock_Tile.m_NoBlockingBelow)
					{
						int num9;
						(tileProperties = m_BuildingLayers[(uint)(m_CurrentLayer - 1)].m_TileProperties)[num9 = num] = tileProperties[num9] | TileProperty.NoBlockingMask;
					}
					else
					{
						int num10;
						(tileProperties = m_BuildingLayers[(uint)(m_CurrentLayer - 1)].m_TileProperties)[num10 = num] = tileProperties[num10] & TileProperty.InverseNoBlockingMask;
					}
				}
			}
		}
	...
	protected TileIDData PlaceWall(int X, int Y, BuildingBlock_Wall obj, int seed, bool bMarkAsChanged = true, bool bReplaceSame = false)
	{
		int num = Y * 120 + X;
		TileIDData tileIDData = (TileIDData)((seed << 22) & 0x3FC00000);
	...
	private void ReplaceWall(int X, int Y, TileIDData previous)
	{
		if (previous != ~TileIDData.ComplexBits)
		{
			int num = Y * 120 + X;
			...
			m_BuildingBlockManager.AdjustLimitationTotal(m_BuildingLayers[(uint)m_CurrentLayer].m_WallTileIDs[num], bAdd: false);
			m_BuildingLayers[(uint)m_CurrentLayer].m_WallTileIDs[num] = previous;
			m_BuildingBlockManager.AdjustLimitationTotal(previous, bAdd: true);
			if ((previous & TileIDData.IDMask) == TileIDData.IDMask)
			{
				int num2;
				TileProperty[] tileProperties;
				(tileProperties = m_BuildingLayers[(uint)m_CurrentLayer].m_TileProperties)[num2 = num] = tileProperties[num2] & TileProperty.InverseWallMask;
				int num3;
				(tileProperties = m_BuildingLayers[(uint)m_CurrentLayer].m_TileProperties)[num3 = num] = tileProperties[num3] & TileProperty.InverseBlockingMask;
			}
			if (m_CurrentComplexProcessed != 0)
			{
				BaseLevelManager.RemoveRoomNumberInProperty(ref m_BuildingLayers[(uint)m_CurrentLayer], num, m_CurrentComplexAllocation);
	...
	protected virtual void PlaceObject(int X, int Y, BuildingBlock_Object obj, bool bMarkAsChanged = true)
	{
		...
		GameObject gameObject = UnityEngine.Object.Instantiate(visualRep, m_BuildingLayers[(uint)m_CurrentLayer].m_Objects.transform);
	...
	protected virtual void RemoveObject(int X, int Y, BuildingBlock_Object obj, bool bMarkAsChanged = true)
	{
		int num = X + obj.m_Footprint.m_iLeft;
		int num2 = Y + obj.m_Footprint.m_iBottom;
		int num3 = Mathf.Clamp(num + obj.m_Footprint.m_iW, 0, 120);
								...
								if (buildingBlock_Tile != null)
								{
									...
									if (applicableTMSVariant != -1)
									{
										...
										if (applicableTMSVariant != num22 || m_BuildingLayers[i].m_TileTileObjects[num5] == null)
										{
											...
											GameObject gameObject = UnityEngine.Object.Instantiate(visualRep, m_BuildingLayers[i].m_Tiles.transform);
								...
								if (buildingBlock_Wall != null)
								{
									...
									if (applicableTMSVariant2 != -1)
									{
										...
										if (applicableTMSVariant2 != num27 || m_BuildingLayers[i].m_WallTileObjects[num5] == null)
										{
											...
											GameObject gameObject3 = UnityEngine.Object.Instantiate(visualRep2, m_BuildingLayers[i].m_Walls.transform);
	...
	public override void AddDelete(ref BuildingInstructionManager.InstructionDeleteElement obj, bool bStorePrevious = true)
	{
		...
		case BuildingInstructionManager.InstructionDeleteElement.DeleteType.Tile:
		{
			...
			TileIDData previous = ValidatePrevious(m_BuildingLayers[(uint)m_CurrentLayer].m_TileTileIDs[num]);
			if (bStorePrevious)
			{
				obj.m_Previous = previous;
			}
		...
		case BuildingInstructionManager.InstructionDeleteElement.DeleteType.Wall:
		{
			...
			TileIDData previous2 = ValidatePrevious(m_BuildingLayers[(uint)m_CurrentLayer].m_WallTileIDs[num]);
			if (bStorePrevious)
			{
	...
	public override void RestoreDelete(ref BuildingInstructionManager.InstructionDeleteElement obj)
	{
		int buildingBrickID = (int)(obj.m_Previous & TileIDData.IDMask);
		int seed = (int)(obj.m_Previous & TileIDData.SeedMask) >> 22;
		switch (obj.m_DeleteType)
		{
		case BuildingInstructionManager.InstructionDeleteElement.DeleteType.Tile:
		{
			BuildingBlock_Tile buildingBlock_Tile = BuildingBlockManager.GetBlock(buildingBrickID) as BuildingBlock_Tile;
			if (!(buildingBlock_Tile == null))
			{
				PlaceTile(obj.m_XPosition, obj.m_YPosition, buildingBlock_Tile, seed);
			}
			break;
		}
		case BuildingInstructionManager.InstructionDeleteElement.DeleteType.Wall:
		{
			...
			if (!(buildingBlock_Wall == null))
			{
				PlaceWall(obj.m_XPosition, obj.m_YPosition, buildingBlock_Wall, seed);
			}
			break;
		}
		case BuildingInstructionManager.InstructionDeleteElement.DeleteType.Object:
		{
			BuildingBlock_Object buildingBlock_Object = BuildingBlockManager.GetBlock(buildingBrickID) as BuildingBlock_Object;
			if (!(buildingBlock_Object == null))
			{
				int x = obj.m_XPosition - buildingBlock_Object.m_Footprint.m_iFirstX - buildingBlock_Object.m_Footprint.m_iLeft;
				int y = obj.m_YPosition - buildingBlock_Object.m_Footprint.m_iFirstY - buildingBlock_Object.m_Footprint.m_iBottom;
				PlaceObject(x, y, buildingBlock_Object);
			}
			break;
		}
		}
	...
	{
		CopyBase();
		EditorLevelEditorManager[] components = GetComponents<EditorLevelEditorManager>();
		for (int i = 0; i < components.Length && !(components[i] != this); i++)
		{
		}
	}
}
